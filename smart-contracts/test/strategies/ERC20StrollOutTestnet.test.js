...
describe("ERC20 Stroll Out Testing (Testnet Deployment)", function () {
  ...
  // Make sure to have 10000 USDC, 10000 DAI and 20 WETH (Aave version)
  ...
  before(async () => {
    ...
    appFactory = await ethers.getContractFactory("ERC20StrollOut", {
      admin,
    });
  });
  ...
  it("should top up an account consuming all allowance", async () => {
    ...
    expect(await USDCContract.allowance(whaleAddr, app.address)).to.equal(
      constants.Zero
    );

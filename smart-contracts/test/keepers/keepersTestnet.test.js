...
const { getBigNumber, getSeconds, getTimeStampNow, increaseTime, impersonateAccounts } = require("../../helpers/helpers");
...
describe("Chainlink Upkeep Testing (Testnet Deployment)", function () {
    ...
    before(async () => {
        ...
        strollHelperFactory = await ethers.getContractFactory("StrollHelper", admin);

        strollHelper = await strollHelperFactory.deploy();

        strollResolverFactory = await ethers.getContractFactory("StrollResolver", admin);

        strollResolver = await strollResolverFactory.deploy(1, getSeconds(5), getSeconds(1));
    ...
    it("should check for upkeep properly", async () => {
        ...
        await amUSDCContract.connect(whale).approve(app.address, parseUnits("100", 6));
        ...
        await registry.connect(whale).createTopUp(USDC.superToken, app.address, USDC.aToken, getBigNumber(getTimeStampNow()).add(getSeconds(90)));
    ...
    it("should perform upkeep consuming all allowance", async () => {
        ...
        expect(getBigNumber(balanceAfterUSDCx).sub(getBigNumber(balanceBeforeUSDCx))).to.be.closeTo(parseUnits("100", 18), parseUnits("1", 18));

        expect(await amUSDCContract.allowance(whaleAddr, app.address)).to.equal(constants.Zero);
    });

    it("should perform upkeep consuming some allowance", async () => {
        await loadFixture(setupEnv);

        await amUSDCContract.connect(whale).approve(app.address, parseUnits("200", 6));
        ...
        expect(getBigNumber(balanceAfterUSDCx).sub(getBigNumber(balanceBeforeUSDCx))).to.be.closeTo(expectedDiff, parseUnits("1", 18));

        expect(await amUSDCContract.allowance(whaleAddr, app.address)).to.be.closeTo(
    ...
    it("should perform upkeep consuming all allowance (multi-token)", async () => {
        ...
        await amUSDCContract.connect(whale).approve(app.address, parseUnits("100", 6));
        await amDAIContract.connect(whale).approve(app.address, parseUnits("100", 18));
        ...
        expect(getBigNumber(balanceAfterUSDCx).sub(getBigNumber(balanceBeforeUSDCx))).to.be.closeTo(parseUnits("100", 18), parseUnits("1", 18));
        expect(getBigNumber(balanceAfterDAIx).sub(getBigNumber(balanceBeforeDAIx))).to.be.closeTo(parseUnits("100", 18), parseUnits("1", 18));

        expect(await amUSDCContract.allowance(whaleAddr, app.address)).to.equal(constants.Zero);
        expect(await amDAIContract.allowance(whaleAddr, app.address)).to.equal(constants.Zero);
    });

    it.only("should perform upkeep consuming some allowance (multi-token)", async () => {
        await loadFixture(setupEnv);

        await amUSDCContract.connect(whale).approve(app.address, parseUnits("200", 6));
        await amDAIContract.connect(whale).approve(app.address, parseUnits("200", 18));
        ...
        expect(getBigNumber(balanceAfterDAIx).sub(getBigNumber(balanceBeforeDAIx))).to.be.closeTo(expectedDiff, parseUnits("1", 18));
        ...
        expect(await amDAIContract.allowance(whaleAddr, app.address)).to.be.closeTo(parseUnits("200", 18).sub(expectedDiffDAI), parseUnits("1", 18));
    });
});
